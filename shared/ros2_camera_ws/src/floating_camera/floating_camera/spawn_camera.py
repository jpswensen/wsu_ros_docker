...
class SpawnCamera(Node):
    def __init__(self):
        ...
        init_pose_vals = self._get_list_param('camera_initial_pose', 6, [0.0, 0.0, 2.0, 0.0, 0.0, 0.0])
        x, y, z, rr, rp, ry = init_pose_vals
        cam_req.initial_pose.position.x = float(x)
        cam_req.initial_pose.position.y = float(y)
        cam_req.initial_pose.position.z = float(z)
        cam_req.initial_pose.orientation = euler_to_quaternion(float(rr), float(rp), float(ry))
        ...
        # Ensure controller starts from the initial pose, then set velocity
        self._controller.set_pose((float(x), float(y), float(z)), (float(rr), float(rp), float(ry)))
    ...
    def _spawn_spheres(self, spawn_cli):
        ...
        # Camera initial pose and orientation
        init_pose_vals = self._get_list_param('camera_initial_pose', 6, [0.0, 0.0, 2.0, 0.0, 0.0, 0.0])
        cx, cy, cz, cr, cp, cyaw = init_pose_vals
        ...
        # Rotation from camera frame to world from RPY
        sr, cr_ = math.sin(cr), math.cos(cr)
        sp, cp_ = math.sin(cp), math.cos(cp)
        sy, cy_ = math.sin(cyaw), math.cos(cyaw)
        # R = Rz(yaw) * Ry(pitch) * Rx(roll)
        R = [
            [cy_ * cp_, cy_ * sp * sr - sy * cr_, cy_ * sp * cr_ + sy * sr],
            [sy * cp_, sy * sp * sr + cy_ * cr_, sy * sp * cr_ - cy_ * sr],
            [-sp,            cp_ * sr,                     cp_ * cr_]
        ]
